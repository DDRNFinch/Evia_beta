...
function removeUnpackedBrickOption(select){
...
function patchSelectors(){document.querySelectorAll("select[data-toc-course]").forEach(removeUnpackedBrickOption)}
...
function ready(){patchSelectors();if(!document.body.__eviaBrickCutoverObserved){document.body.__eviaBrickCutoverObserved=true;new MutationObserver(schedule).observe(document.body,{childList:true,subtree:true})}}
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",ready);else ready();
window.addEventListener("pageshow",ready);
window.EviaBrickPackCutover={packInstalled,patchSelectors};
})();